...
class ProductControllerIntegrationTest {
	...
	@BeforeEach
	void setUp() {
		userRepository.findByUsername("admin").orElseGet(() -> userRepository.save(User.builder().username("admin")
				.password(passwordEncoder.encode("Admin@123")).role(User.Role.ROLE_ADMIN).build()));
	}
	...
	void fullCrudLifecycle_worksEndToEnd() throws Exception {
		...
		String createResponse = mockMvc
				...
				.andExpect(status().isCreated()).andExpect(jsonPath("$.productName").value("Mechanical Keyboard"))
		...
		mockMvc.perform(get("/api/v1/products/" + id).header("Authorization", "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$.productName").value("Mechanical Keyboard"));

		// READ (paginated list)
		mockMvc.perform(get("/api/v1/products?page=0&size=10").header("Authorization", "Bearer " + token))
				.andExpect(status().isOk()).andExpect(jsonPath("$.content").isArray());

		// UPDATE
		ProductRequest updateRequest = new ProductRequest("Mechanical Keyboard RGB");
		mockMvc.perform(put("/api/v1/products/" + id).header("Authorization", "Bearer " + token)
				.contentType("application/json").content(objectMapper.writeValueAsString(updateRequest)))
				.andExpect(status().isOk()).andExpect(jsonPath("$.productName").value("Mechanical Keyboard RGB"));
	...
	void createProduct_withBlankName_returnsBadRequest() throws Exception {
		...
		mockMvc.perform(post("/api/v1/products").header("Authorization", "Bearer " + token)
				.contentType("application/json").content(objectMapper.writeValueAsString(new ProductRequest(""))))
